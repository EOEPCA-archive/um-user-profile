...
class OAuthClient():
    ...
    def get_login_url(self):
        auth_endpoint = self.wkhandler.get(WellKnownHandler.TYPE_OIDC, WellKnownHandler.KEY_OIDC_AUTHORIZATION_ENDPOINT)

        return auth_endpoint + "?scope="+self.scopes+"&client_id="+self.client_id+"&redirect_uri="+self.redirect_uri+"&response_type=code"

    def get_token(self, code):
        token_endpoint = self.wkhandler.get(WellKnownHandler.TYPE_OIDC, WellKnownHandler.KEY_OIDC_TOKEN_ENDPOINT)
    ...
    def get_user_info(self,access_token):
        user_info_endpoint = self.wkhandler.get(WellKnownHandler.TYPE_OIDC, WellKnownHandler.KEY_OIDC_USERINFO_ENDPOINT)
    ...
    def end_session_url(self, id_token):
        end_session_endpoint = self.wkhandler.get(WellKnownHandler.TYPE_OIDC, WellKnownHandler.KEY_OIDC_END_SESSION_ENDPOINT)
